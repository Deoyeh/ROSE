...
def main():
    ...
    #e.g. if using LSF cmd1 = "bsub bamToGFF.py -f 1 -e 200 -r -m %s -b %s -i %s -o %s" % (nBin,bamFile,stitchedGFFFile,mappedOut1)
    ...
    for bamFile in bamFileList:
        ...
        cmd1 = "ROSE_bamToGFF_turbo.py -e 200 -r -m %s -b %s -i %s -o %s &" % (nBin,bamFile,stitchedGFFFile,mappedOut1)
        print(cmd1)
        os.system(cmd1)
        ...
        #WILL TRY TO RUN AS A BACKGROUND PROCESS. BATCH SUBMIT THIS LINE TO IMPROVE SPEED
        cmd2 = "ROSE_bamToGFF_turbo.py 1 -e 200 -r -m %s -b %s -i %s -o %s &" % (nBin,bamFile,inputGFFFile,mappedOut2)
        print(cmd2)
        os.system(cmd2)
    ...
    if options.control:
        ...
        cmd = 'ROSE_callSuper.R --no-save %s %s %s %s' % (outFolder,outputFile1,inputName,controlName)
    ...
    cmd = "ROSE_geneMapper.py -g %s -i %s%s" % (genome,outFolder,superTableFile)
    os.system(cmd)    
